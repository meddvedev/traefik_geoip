...
func newCityDBLookup(rdr *geoip2.CityReader, lang string) LookupGeoIP {
	return func(ip net.IP) (*GeoIPResult, error) {
		...
		if country, ok := rec.Country.Names[lang]; ok {
			retval.country = country
		}
		if city, ok := rec.City.Names[lang]; ok {
			retval.city = city
		}
...
func newCountryDBLookup(rdr *geoip2.CountryReader, lang string) LookupGeoIP {
	return func(ip net.IP) (*GeoIPResult, error) {
		rec, err := rdr.Lookup(ip)
		if err != nil {
			return nil, fmt.Errorf("%w", err)
...
func NewLookup(dbPath string, lang string) (LookupGeoIP, error) {
	...
	case strings.Contains(dbPath, "City"):
		...
		lookup = newCityDBLookup(rdr, lang)
	...
	case strings.Contains(dbPath, "Country"):
		...
		lookup = newCountryDBLookup(rdr, lang)
